...
public class C_Operator {
	...
	public static void main(String[] args) {
		...
		//산술 연산자: 사칙연산 직접 숫자 데이터에 대해서 연산을 수행
		//더하기 : +
		//좌항에 우항을 더한 값을 반환
		...
		doubleResult = c + d;
		doubleResult = a + d;
		intResult =(int) (c + d); //형 변환 해줘야 함
		//intResult = a + d; //출력 안됨 정수 + 실수는 실수가 나옴
		doubleResult = a + d; //출력 안됨 정수 + 실수는 실수가 나옴
		...
		intResult = a / b;    //정수끼리만 나누면 정수만 나옴
		System.out.println(intResult);
		doubleResult = c / d; 
		System.out.println(doubleResult);
		doubleResult = a / d; // 실수와 정수를 나누면 실수가 나옴
		System.out.println(doubleResult);
		...
		//대입 연산자 : 좌항에 우항을 할당
		// 일반 대입 연산자 : =
		// 상수에 초기화를 할때를 제외하고 좌항이 반드시 변수여야 함
		// 좌항에 우항을 할당하는 역활을 함
		number = 10;
		...
		//int number1 += 10; 변수를 초기화 시키지 않으면 컴파일러 오류
		
		// 비교 연산자 : 좌항과 우항을 비교하여 그 결과를 논리값으로 반환
		boolean booleanResult; //선언
		...
		booleanResult = false && false;    //무조건 false로 읽기에 뒷작업은 생각지 않음
		System.out.println(booleanResult);
		booleanResult = a > b && c > d;
		System.out.println(booleanResult);
		
		// || : OR  연산 - 좌항과 우항 중 하나라도 true면 true, 모두 false면 false
		booleanResult = true || true;     //무조건 true로 읽기에 뒷작업은 생각지 않음
		System.out.println(booleanResult);
		System.out.println(booleanResult);
		...
		// 삼항 연산자 : 간단한 조건에 따라 결과를 변환하는 연산자
		// 조건(논리식) ? 참일때결과 : 거짓일때결과
